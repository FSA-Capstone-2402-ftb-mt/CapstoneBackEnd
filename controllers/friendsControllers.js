import {sendFriendRequest, acceptFriendRequest, rejectFriendRequest, getFriends } from '../models/friends.js';
...
export const sendFriendRequestController = async (req, res) => {
    try {
        ...
        const friendRequest = await sendFriendRequest(user_username, friend_username);
...
export const acceptFriendRequestController = async (req, res) => {
    try {
        const { user_username, friend_username } = req.body;
        const friendship = await acceptFriendRequest(user_username, friend_username);
...
export const rejectFriendRequestController = async (req, res) => {
    try {
        const { user_username, friend_username } = req.body;
        const friendship = await rejectFriendRequest(user_username, friend_username);
        res.status(200).json({ message: 'Friend request rejected successfully', friendship });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
};
