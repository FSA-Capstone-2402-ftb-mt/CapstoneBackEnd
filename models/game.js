...
import { calculateScore, calculateTimedScore } from '../services/gameService.js';

// Function to update regular game score
export const updateRegularGameScore = async (username, correctGuess, attempts, streak) => {
    try {
...
export const updateTimedGameScore = async (username, correctGuess, attempts, timeTaken, streak) => {
    try {
        // Fetch the current streak from the database
        const { rows: userRows } = await client.query(`
            SELECT current_streak, max_streak
        ...
        // Calculate the score
        const score = calculateTimedScore(correctGuess, attempts, timeTaken, streak);
